...
Вписываем нужный storageClass, чтобы установленный ранее авто провижионер https://github.com/rancher/local-path-provisioner  выдал нам нужные PV
...
правим количество реплик, пока делаем 1
...
Редактируем еще раз values.yml, теперь реплик две
...
Все работает со стороны клиента. Был обрыв соединения т.к. мы переключили порт.

![alt text](image-19.png)

Выводы: Мы научились управлять k8s как напрямую через kubectl, так и через helm, посмотрели как можно быстро поднять готовый кластер, и как много настроек он имеет. 
Коротко изучили концепцию PVC и PV, посмотрели как это работает на практике