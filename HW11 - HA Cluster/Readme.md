...
Только добавим в него нормлаьную отказоустойчивость самого HAProxy с помощью keepalived
...
1. Восстанавливаем наш отказуостойчивый кламтер, запускаем службу патрони, удаляем папку с данными из работы про кликхаус.
Убеждаемся, что кластер работает и у нас есть две реплики.
...
```
Получаем порты для мастера и реплик с балансировкой - роунд-робин
...
В /etc/keepalived/haproxy_check.sh записываем
